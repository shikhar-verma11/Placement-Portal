...
export const HorizontalWorkflowSection: React.FC = () => {
  ...
  return (
    <section id="workflow" ref={sectionRef} className="relative h-screen bg-[#071320] overflow-hidden flex flex-col justify-center">
      {/* Background Lighting & Connecting Cable Glow */}
      <div className="absolute top-1/2 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-[#14F1D9]/40 to-transparent pointer-events-none" />

      {/* Section Title */}
      <div className="absolute top-8 left-6 sm:left-12 z-20">
        <div className="inline-flex items-center gap-2 px-4 py-1 rounded-full bg-[#0F172A] border border-[#14F1D9]/30 text-xs font-mono text-[#00FFC6] mb-2">
          <Network className="w-3.5 h-3.5" />
          <span>PINNED WORKFLOW TIMELINE</span>
        </div>
      ...
      >
        {workflowSteps.map((item, idx) => (
          <div
            ...
            className="group relative w-80 sm:w-96 glass-panel rounded-3xl p-6 sm:p-8 border-[#14F1D9]/20 bg-[#0F172A]/90 hover:border-[#00FFC6] transition-all duration-300 flex-shrink-0 flex flex-col justify-between"
            data-cursor-hover
            data-cursor-text="STEP"
          >
            {/* Glowing Step Connector Badge */}
            <div className="flex items-center justify-between mb-6">
              <span className="w-10 h-10 rounded-2xl bg-[#00FFC6]/20 border border-[#00FFC6] flex items-center justify-center font-mono font-extrabold text-[#00FFC6] text-sm shadow-[0_0_15px_rgba(0,255,198,0.3)]">
                {item.step}
              </span>
            ...
            <div>
              <h3 className="text-xl font-heading font-bold text-white mb-2 group-hover:text-[#00FFC6] transition-colors">
                {item.title}
              </h3>
            ...
            <div className="pt-4 border-t border-white/10 flex items-center justify-between text-xs font-mono">
              <span className="text-[#00FFC6] font-bold flex items-center gap-1.5">
                <CheckCircle2 className="w-3.5 h-3.5" /> {item.stats}
              </span>
              {idx < workflowSteps.length - 1 && (
                <ArrowRight className="w-4 h-4 text-slate-500 group-hover:text-[#00FFC6] group-hover:translate-x-1 transition-transform" />
              )}
            </div>
          </div>
        ))}
